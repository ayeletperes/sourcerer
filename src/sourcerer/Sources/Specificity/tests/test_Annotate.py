...
class TestHandleAnnotate(unittest.TestCase):
    ...
    def _baseArgs(self, **overrides):
        defaults = dict(compare_cols=['cdr3_aa'], max_mismatches=0,
                        fraction=False, exact_cols=None,
                        id_col='sequence_id', vgene_col=None, jgene_col=None,
                        compairr_bin=None, threads=1, locus=None)
    ...
    def test_fuzzy_annotation_still_works_without_compairr(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._writeTsv(query_path, [{'sequence_id': 'q1', 'cdr3_aa': 'CARDYW'}])
            self._writeTsv(reference_path,
                           [{'sequence_id': 'ref1', 'cdr3_aa': 'CARQYW'}])
    ...
    def test_fraction_flag_routes_through_the_fraction_threshold(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp = Path(tmp)
            query_path = tmp / 'query.tsv'
            reference_path = tmp / 'reference.tsv'
            ...
            self._writeTsv(query_path, [{'sequence_id': 'q1', 'cdr3_aa': 'AAAAAAAAAA'}])
            self._writeTsv(reference_path,
                           [{'sequence_id': 'ref1', 'cdr3_aa': 'AAAAAAAAAB'}])

            handleAnnotate(self._baseArgs(
                query=query_path, reference=reference_path, out=out_path,
                max_mismatches=0.15, fraction=True))
